 ...
 */   object ColorConverter {
    fun convert(`in`: SettingsColor?): Color {
        return Color(`in`.getR(), `in`.getG(), `in`.getB())
    }

    fun convert(`in`: Color): SettingsColor {
        return SettingsColor(`in`.getRed(), `in`.getGreen(), `in`.getBlue())
    }

    fun convertToAwt(`in`: List<SettingsColor?>?): MutableList<Color> {
        return `in`!!.stream().map(Function<SettingsColor?, Color>({ obj: SettingsColor? -> convert() }))
            .collect(Collectors.toList())
    }

    fun convertToSettings(`in`: List<Color?>?): List<SettingsColor> {
        return `in`!!.stream().map(Function<Color?, SettingsColor>({ obj: Color? -> convert() }))
            .collect(Collectors.toList())
    }
}