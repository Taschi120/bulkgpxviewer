...
class GpxFile constructor(val fileName: Path, private var gpx: GPX) {
    ...
    fun save() {
        doBackup()
        GpxFile.log.info("Writing GPX to '{}'", fileName) //$NON-NLS-1$
        GPX.write(getGpx(), fileName)
        GpxFile.log.info("Written GPX to '{}' successfully", fileName) //$NON-NLS-1$
        changed = false
    }
    ...
    private fun doBackup() {
        ...
        GpxFile.log.info("Creating backup of '{}' at '{}'", `in`, out) //$NON-NLS-1$
