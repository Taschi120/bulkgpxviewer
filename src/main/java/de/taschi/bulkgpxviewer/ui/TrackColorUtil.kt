...
class TrackColorUtil constructor() {
    ...
    fun getColorForTrack(track: GpxFile?): Color? {
        val settingColors: List<SettingsColor> =
            Collections.unmodifiableList(settingsManager.getSettings().getRouteColors())
        val colors: List<Color?>? = ColorConverter.convertToAwt(settingColors)
        if (colors!!.isEmpty()) {
            TrackColorUtil.log.error("List of available colors is empty") //$NON-NLS-1$
            return Color.RED
        }
