...
class MapSelectionHandler(private val target: JXMapViewer) : MouseAdapter() {
    ...
    override fun mouseClicked(e: MouseEvent) {
        ...
        if (wp != null) {
            if (selectedPoints.contains(wp)) {
                MapSelectionHandler.log.info("Deselecting waypoint at {}|{}", wp.latitude, wp.longitude) //$NON-NLS-1$
                selectedPoints.remove(wp)
            } else if (selectedPoints.size < 2) {
                MapSelectionHandler.log.info("Selecting waypoint at {}|{}", wp.latitude, wp.longitude) //$NON-NLS-1$
    ...
    private fun getNearestWaypointToMouseCoords(mouse: Point2D): WayPoint? {
        ...
        if (points!!.isEmpty()) {
            MapSelectionHandler.log.warn("Track is empty, cannot find nearest point") //$NON-NLS-1$
            return null
        }
    ...
    fun activate(track: GpxFile?) {
        MapSelectionHandler.log.info("Entering edit mode for track {}", track.getFileName()) //$NON-NLS-1$
        this.track = track
        active = true
    }
     ...
     */
    fun deactivate() {
        MapSelectionHandler.log.info("Exiting edit mode") //$NON-NLS-1$
        track = null
        active = false
