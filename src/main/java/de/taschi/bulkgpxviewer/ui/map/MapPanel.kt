...
class MapPanel : JPanel() {
    ...
    @Getter
    private var selectedFile: GpxFile? = null
    ...
    fun autoSetZoomAndLocation() {
        ...
        if (tracks!!.isEmpty()) {
            ...
            tracks.stream().map { obj: GpxFile? -> obj!!.gpx }
                .flatMap { obj: GPX? -> obj!!.tracks() }
                ...
                .forEach { p: GeoPosition -> bb.clamp(p) }
            mapKit.addressLocation = GeoPosition(bb.centerLat, bb.centerLong)
            mapKit.setZoom(8)
        }
    }
     ...
     * @param selectedFile
     */
    fun setSelectedFile(selectedFile: GpxFile?) {
