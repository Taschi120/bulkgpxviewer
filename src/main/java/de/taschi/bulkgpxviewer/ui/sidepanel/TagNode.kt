...
class TagNode(parent: GpxFileTreeNode?) : DefaultMutableTreeNode(), GpxFileRelatedNode {
    @Inject
    private val tagManager: TagManager? = null
    private val parent: GpxFileTreeNode
    ...
    override fun update() {
        val tags = tagManager!!.getTagsForGpxFile(parent.getGpxFile())
        val format = "Tags: %s"
        var tagList: String? = "none"
        if (!tags!!.isEmpty()) {
            tagList = tags.stream()
                .map { tag: Tag? -> tagManager.getLocalizedName(tag) }
