 ...
 * #L%
 */   class GpxToJxMapper constructor() {
    fun waypointToGeoPosition(`in`: WayPoint): GeoPosition {
        return GeoPosition(`in`.getLatitude().toDouble(), `in`.getLongitude().toDouble())
    }

    companion object {
        private var INSTANCE: GpxToJxMapper? = null
        val instance: GpxToJxMapper?
            get() {
                if (INSTANCE == null) {
                    INSTANCE = GpxToJxMapper()
                }
                return INSTANCE
            }
    }
