 ...
 * #L%
 */   class SpeedCalculator constructor() {
    @Inject
    ...
    fun getAverageSpeed(gpx: GPX, unitSystem: UnitSystem): Double {
        ...
        val optionalTime: Optional<Duration?>? = durationCalculator!!.getRecordedDurationForGpxFile(gpx)
        if (optionalTime!!.isPresent()) {
            val time: Long = optionalTime.get().getSeconds()
            if (time == 0L) {
                return Double.NaN
            } else {
