...
class DurationCalculator constructor() {
    ...
    fun getRecordedDurationForGpxFile(gpx: GPX): Optional<Duration?> {
        ...
        for (track: Track in gpx.getTracks()) {
            val duration: Optional<Duration?> = getRecordedDurationForTrack(track)
            if (duration.isEmpty()) {
                return duration
            } else {
    ...
    fun getRecordedDurationForTrack(track: Track): Optional<Duration?> {
        ...
        for (segment: TrackSegment in track.getSegments()) {
            val duration: Optional<Duration?> = getSegmentDuration(segment)
            if (duration.isEmpty()) {
                return duration
            } else {
                result = result.plus(duration.get())
     ...
     */
    fun getSegmentDuration(segment: TrackSegment): Optional<Duration?> {
        ...
        val start: Instant? = waypoints.get(0).getInstant().orElse(null)
        val end: Instant? = waypoints.get(waypoints.size - 1).getInstant().orElse(null)
