 ...
 */   class RouteLengthCalculator constructor() {
    ...
    fun getFormattedTotalDistance(gpx: GPX, system: UnitSystem?): String {
        ...
        val unit: String?
        when (system) {
            UnitSystem.IMPERIAL -> unit = Messages.getString("RouteLengthCalculator.UnitMiles")
            UnitSystem.METRIC -> unit = Messages.getString("RouteLengthCalculator.UnitKilometers")
            else -> unit = Messages.getString("RouteLengthCalculator.UnitKilometers")
        }
    ...
    fun getTotalDistance(gpx: GPX): Double {
        return gpx.getTracks().stream()
            .map(Function({ track: Track -> this.getTotalDistance(track) }))
            .reduce(0.0, BinaryOperator({ a: Double, b: Double -> java.lang.Double.sum(a, b) }))
    }

    fun getTotalDistance(track: Track): Double {
        return track.getSegments().stream()
            .map(Function({ segment: TrackSegment -> this.getTotalDistance(segment) }))
            .reduce(0.0, BinaryOperator({ a: Double, b: Double -> java.lang.Double.sum(a, b) }))
    }
    ...
    fun getTotalDistance(segment: TrackSegment): Double {
        ...
        if (points.size < 2) {
            ...
            for (i in 0 until (points.size - 1)) {
                val here: WayPoint = points.get(i)
                val there: WayPoint = points.get(i + 1)
